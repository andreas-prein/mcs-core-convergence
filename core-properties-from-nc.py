...
dbz_limit = 20 # minimum dbz threshold to consider cores
...
outdir = '/glade/campaign/mmm/c3we/prein/Papers/2023_Conv-Cores/data/core_stats'+skip_ml+'_20dBZ/'+SIM_All[si]+'/'
...
for ii in tqdm(range(len(lat_vs))):
    ...
    for ca in range(len(conv_inters)):
        ...
        for fi in [0]: #range(len(files_int)):
            ...
            cloudmask[0,:] = np.nan
            cloudmask[-1,:] = np.nan
            cloudmask[dbz_ca_comz[:,dx_dist,:] <= dbz_limit] = np.nan
            
            cloudmask_3d = np.copy(dbz_ca_comz[:,:,:]); cloudmask_3d[:] = 1
            cloudmask_3d[dbz_ca_comz[:,:,:] <= dbz_limit] = np.nan
        ...
        if subkm == True:                
            ...
            for th in range(len(draft_th)):
                for di in range(len(direction)):
                    ...
                    cloudmask[0,:] = np.nan
                    cloudmask[-1,:] = np.nan
                    cloudmask[wrf_dbz_column[0] <= dbz_limit] = np.nan
